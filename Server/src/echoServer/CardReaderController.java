...
public class CardReaderController implements Initializable {
		...
		if (amountActual.getText().length() > 0) {
			...
			if (onlyNumber(amountActual.getText())) {
				...
				if (amountActualInt > maxAmount || amountActualInt == 0) {
					...
					mysqlConnection.updateToActive(arrtmp);
	...
	void enterCardsimulate(ActionEvent event) {
		...
		arr= mysqlConnection.simulationCardReader();
		if (arr.size() > 0) {
			userIDH = arr.get(0);
		...
		else NoExistOrderMsg.setVisible(true);
	}
	...
	public void getToTable() {
		...
		ArrayList<OrderToView> temp = mysqlConnection.ReturnUserIDInTableOrdersForCardReader(arr);
		...
		if (!temp.isEmpty()) {
			...
			for (int i = 0; i < temp.size(); i++) {
				...
				//	arr.add("ViewOrder");
					...
					arr =  mysqlConnection.ViewOrders(arr);
			...
			tblExistingOrder.setRowFactory(tv -> {
				...
				//		arrtmp.add("ViewOrder");
						...
						arrtmp = mysqlConnection.ViewOrders(arrtmp);
